...
def glvq_mu(distances: torch.Tensor, proto_labels: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
...
class TransferGMLVQ(nn.Module):
    ...
    def __init__(
        ...
        self.class_protos = nn.Parameter(
            torch.randn(n_classes * ppc, n_features, generator=g) * 0.1
        )
        self.source_protos = nn.Parameter(
            torch.randn(n_sources * pps, n_features, generator=g) * 0.1
        )
    ...
    def complementary_projector(self) -> torch.Tensor:
        ...
        n = self.n_features
        return (
            torch.eye(n, device=self.omega.device, dtype=self.omega.dtype)
            - self.omega.t() @ self.omega
        )
    ...
    def loss(
        self, x: torch.Tensor, y_class: torch.Tensor, y_source: torch.Tensor, alpha: float, f=None
    ) -> torch.Tensor:
